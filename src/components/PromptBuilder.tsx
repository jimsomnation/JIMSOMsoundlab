...
export const PromptBuilder: React.FC<PromptBuilderProps> = ({
  ...
  const renderSelector = (sectionId: string, type: 'mood' | 'gender' | 'theme' | 'timestamp' | 'bpm' | 'scale') => {
    ...
    return (
      <div className="absolute top-full left-0 mt-2 z-[100] bg-gray-950 rounded-2xl shadow-2xl border border-gray-800 p-2 min-w-[200px] animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
        <div className="space-y-1 max-h-48 overflow-y-auto custom-scrollbar">
          {options.map(opt => (
            <button 
              ...
              className={`w-full text-left px-4 py-2 rounded-xl text-sm transition-colors ${currentValue === opt ? 'bg-blue-950/60 text-blue-400 font-bold' : 'hover:bg-gray-900 text-slate-300'}`}
            >
              {opt}
            </button>
          ))}
        </div>
        <div className="border-t border-gray-850 mt-2 pt-2">
          <div className="flex gap-2 p-1">
            <input 
              type="text" 
              placeholder="Custom..." 
              className="flex-1 bg-gray-900 border-none rounded-lg px-3 py-1.5 text-xs text-white focus:ring-1 focus:ring-blue-500 placeholder-slate-500"
            ...
            <button 
              ...
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold rounded-lg"
            >
              Set
            </button>
          </div>
      ...
      <div className="relative">
        {value ? (
          <button 
            ...
            className={`px-3 py-1 bg-blue-950/40 border border-blue-900/50 rounded-lg text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 hover:bg-blue-950/60 transition-all text-sm group/pill`}
          >
          ...
          <button 
            onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === type ? null : { sectionId: s.id, type }); }}
            className="px-3 py-1 bg-gray-950 border border-dashed border-gray-800 rounded-lg text-slate-400 font-medium hover:border-blue-500 hover:text-blue-400 transition-all text-xs flex items-center gap-1.5"
          >
  ...
  return (
    <div className="w-full bg-blue-950/10 border border-blue-900/30 rounded-3xl p-8 space-y-6 transition-all" onClick={e => e.stopPropagation()}>
      {helperSections.map((s) => (
        <div key={s.id} className="flex flex-wrap items-center gap-x-3 gap-y-4 text-xl font-light leading-relaxed">
          {s.type === 'main' ? (
            <>
              <span className="text-slate-400">Create a</span>
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === 'mood' ? null : { sectionId: s.id, type: 'mood' }); }} className={`px-4 py-1.5 bg-gray-950 border rounded-xl text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 transition-all ${activeSelector?.sectionId === s.id && activeSelector?.type === 'mood' ? 'border-blue-500 ring-2 ring-blue-950' : 'border-blue-900/50'}`}>
                  {s.mood} <Icons.ChevronDown className="w-3.5 h-3.5 opacity-50" />
                </button>
                {renderSelector(s.id, 'mood')}
              </div>
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === 'gender' ? null : { sectionId: s.id, type: 'gender' }); }} className={`px-4 py-1.5 bg-gray-950 border rounded-xl text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 transition-all ${activeSelector?.sectionId === s.id && activeSelector?.type === 'gender' ? 'border-blue-500 ring-2 ring-blue-950' : 'border-blue-900/50'}`}>
                  {s.gender} <Icons.ChevronDown className="w-3.5 h-3.5 opacity-50" />
                </button>
                {renderSelector(s.id, 'gender')}
              </div>
              <span className="text-slate-400">song about</span>
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === 'theme' ? null : { sectionId: s.id, type: 'theme' }); }} className={`px-4 py-1.5 bg-gray-950 border rounded-xl text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 transition-all ${activeSelector?.sectionId === s.id && activeSelector?.type === 'theme' ? 'border-blue-500 ring-2 ring-blue-950' : 'border-blue-900/50'}`}>
            ...
            <>
              ...
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === 'timestamp' ? null : { sectionId: s.id, type: 'timestamp' }); }} className="px-3 py-1 bg-gray-950 border border-blue-900/50 rounded-lg text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 transition-all text-sm">
                  {s.timestamp} <Icons.ChevronDown className="w-3 h-3 opacity-50" />
                </button>
                {renderSelector(s.id, 'timestamp')}
              </div>
              <span className="text-slate-400">] the song transitions to a </span>
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === 'mood' ? null : { sectionId: s.id, type: 'mood' }); }} className="px-3 py-1 bg-gray-950 border border-blue-900/50 rounded-lg text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 transition-all text-sm">
                  {s.mood} <Icons.ChevronDown className="w-3 h-3 opacity-50" />
                </button>
                {renderSelector(s.id, 'mood')}
              </div>
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setActiveSelector(activeSelector?.sectionId === s.id && activeSelector?.type === 'gender' ? null : { sectionId: s.id, type: 'gender' }); }} className="px-3 py-1 bg-gray-950 border border-blue-900/50 rounded-lg text-blue-400 font-bold shadow-sm flex items-center gap-2 hover:border-blue-500 transition-all text-sm">
              ...
              {renderOptionalValuePill(s, 'scale', 'Add Scale')}
              <button onClick={(e) => { e.stopPropagation(); setHelperSections(prev => prev.filter(sec => sec.id !== s.id)); }} className="text-slate-500 hover:text-red-400 transition-colors p-1 ml-auto"><Icons.X className="w-5 h-5" /></button>
            </>
          )}
        </div>
      ))}
        ...
        {selectedImages.length > 0 && (
          <div className="flex flex-wrap gap-4">
            {selectedImages.map((img, idx) => (
              <div key={idx} className="relative group rounded-2xl overflow-hidden border border-gray-800 w-24 h-24 flex-shrink-0 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          {selectedImages.length < 10 && (
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl font-bold transition-all border shadow-sm bg-gray-950 text-blue-400 border-blue-900/40 hover:bg-blue-950/60 hover:border-blue-900`}
            >
          ...
          <button 
            ...
            className="flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl bg-gray-950 text-blue-400 border border-blue-900/40 font-bold shadow-sm hover:bg-blue-950/60 hover:border-blue-900 transition-all"
          >
